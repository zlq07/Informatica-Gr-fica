...
void vista_orto()
{
    ...
    glViewport(0, Ancho / 2, Ancho / 2, Alto / 2);
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glOrtho(-5 * factor, 5 * factor, -5 * factor, 5 * factor, -100, 100);
    glRotatef(90, 1, 0, 0);
...
void procesar_color(unsigned char color[3])
{
    ...
    if (t_objeto == SCOOTER)
    {
        ...
        for (i = 0; i < scooter.piezas; i++)
        {
            if (color[0] == scooter.color_select[i].r &&
                color[1] == scooter.color_select[i].g &&
                color[2] == scooter.color_select[i].r)
            {
                if (scooter.activo[i] == 0)
                {
                    scooter.activo[i] = 1;
                }
                else
